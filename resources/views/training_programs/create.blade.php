
@extends('layouts.app')
@section('title', 'Create Training Program')

@section('content')
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
<style>
    :root { --primary-color: #4f46e5; --light-bg: #f5f7fa; --card-bg: #ffffff; --border-color: #e5e7eb; }
    .container-custom { padding: 20px 0; background: var(--light-bg); }
    .form-control-modern, .form-select-modern {
        border-radius: .5rem; border: 1px solid var(--border-color); padding: .6rem .8rem;
        font-size: .9rem; min-height: 40px;
    }
    .form-section-panel {
        background: var(--card-bg); border: 1px solid var(--border-color);
        border-radius: .75rem; padding: 2rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,.05);
    }
    .section-title { font-size: 1.5rem; font-weight: 700; border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; margin-bottom: 1.5rem; }
</style>

<div class="container container-custom">
    <div class="row justify-content-center">
        <div class="col-md-10 col-lg-9">
            <a href="{{ route('training_programs.index') }}" class="btn btn-outline-secondary rounded-pill mb-4">
                Back to Programs
            </a>

            <div class="form-section-panel">
                <h4 class="section-title">Create New Training Program</h4>

                <form action="{{ route('training_programs.store') }}" method="POST">
                    @csrf
                    <div class="row g-4">

                        <div class="col-12">
                            <label class="form-label">Program Name <span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control-modern @error('name') is-invalid @enderror"
                                   value="{{ old('name') }}" required>
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-12">
                            <label class="form-label">Details (Optional)</label>
                            <textarea name="details" class="form-control-modern" rows="3">{{ old('details') }}</textarea>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label">Instructor <span class="text-danger">*</span></label>
                            <select name="instructor_employee_id" class="form-select-modern instructor-select-ajax" required>
                                <option value="" disabled selected>Search instructor...</option>
                            </select>
                            @error('instructor_employee_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-6">
                            <label class="form-label">Available Time <span class="text-danger">*</span></label>
                            <input type="text" name="available_time" class="form-control-modern" 
                                   value="{{ old('available_time') }}" placeholder="e.g. 5:00pm - 7:00pm" required>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label">Available Days <span class="text-danger">*</span></label>
                            <select name="available_days[]" class="form-select-modern available-days-select" multiple required>
                                @foreach(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $day)
                                    <option value="{{ $day }}" {{ in_array($day, old('available_days', [])) ? 'selected' : '' }}>
                                        {{ $day }}
                                    </option>
                                @endforeach
                            </select>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label">Max Participants <span class="text-danger">*</span></label>
                            <input type="number" name="available_total_employees" class="form-control-modern"
                                   value="{{ old('available_total_employees') }}" min="1" required>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label">Status <span class="text-danger">*</span></label>
                            <select name="status" class="form-select-modern" required>
                                <option value="available" {{ old('status') == 'available' ? 'selected' : '' }}>Available</option>
                                <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="ended" {{ old('status') == 'ended' ? 'selected' : '' }}>Ended</option>
                            </select>
                        </div>
                    </div>

                    <div class="text-end mt-5">
                        <button type="submit" class="btn btn-primary btn-lg px-5 rounded-pill">
                            Create Program
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>

<script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
<script>
$(document).ready(function() {
    $('.instructor-select-ajax').select2({
        placeholder: 'Search instructor...',
        allowClear: true,
        ajax: {
            url: "{{ route('training_programs.search_employees') }}",
            dataType: 'json',
            delay: 250,
            data: params => ({ q: params.term }),
            processResults: data => ({ results: data.results }),
            cache: true
        }
    });

    $('.available-days-select').select2({
        placeholder: 'Select days',
        allowClear: true
    });
});
</script>
@endsection
